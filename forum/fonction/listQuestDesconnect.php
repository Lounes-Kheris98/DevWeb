<?php
      ...
      if (!isset($_GET["page"]) || (strcmp($_GET["page"],"1")==0)){
        try{
         $pdo_options[PDO::ATTR_ERRMODE] = PDO::ERRMODE_EXCEPTION;
         $bdd = new PDO('mysql:host=localhost;dbname=project', 'root', '', $pdo_options);
         $bdd->exec('set names utf8');
        ...
        try{
         ...
         echo "<div id='listRep'>";
         $msgParPage=5;
         $rep = $bdd->prepare('SELECT COUNT(id_rep) AS nbrep FROM reponse r, question q WHERE r.id_question=q.id_question AND nom_domaine=? AND r.id_question=? AND r.supprime=0');
         $rep->execute(array($_GET["domaine"],$_GET["id_quest"]));
         $result = $rep->fetch(); $total=intval($result["nbrep"]);
